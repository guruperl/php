var SearchToObject = function(search) {
  var pairs = search.substring(1).split("&"), obj = {}, pair, i;
  for (i in pairs) {
    if (pairs[i]==="") continue;
    pair = pairs[i].split("=");
...
class Genelet {
  constructor(args) {
    ...
    this.OTHER  = {};
    this.loc    = {};
    this.partial_header = args.role+"/"+args.header+"."+args.html;
    this.partial        = args.role+"/"+args.logins+"."+args.html;
    this.partial_footer = args.role+"/"+args.footer+"."+args.html;
  }
  ...
  updateParameters(role, comp, action) {
    ...
    this.partial_header = role+"/"+this.header+"."+this.html;
    if (action===undefined) this.partial = role+"/"+comp+"."+this.html;
    else  this.partial = role+"/"+comp+"/"+action+"."+this.html;
    this.partial_footer = role+"/"+this.footer+"."+this.html;
  };

  start() {
    this.initURL();
    this.ajaxPage(this.role, this.comp, SearchToObject(this.loc.search), "GET");
  };

  initURL() {
    var str_hash = "/"+this.role+"/"+this.json+"/"+this.comp+"?action="+this.action
    if (location.hash && location.hash.length>1) {
        var parts = location.hash.substring(1).split("/");
		this.role = parts[1]
		this.comp = parts[2].split("?")[0]
        parts.splice(2,0,this.json)
        str_hash = parts.join("/")
    }
    this.loc = new URL(this.siteUrl+str_hash, location.origin);
  };
...
console.log(data)
    ...
    if (landing===undefined) {
      ...
      this.loc.search = "";
      if (method==='GET') {
        this.loc.search = "?" + Object.keys(query).map((key) => { return encodeURIComponent(key) + '=' + encodeURIComponent(query[key]) }).join('&');
      }
      this.updateParameters(role, comp, query.action);
      parent.location.hash = "/"+role+"/"+comp+this.loc.search;
console.log("Done. PATH: "+"/"+role+"/"+comp+this.loc.search);
      return true;
    }
...
console.log("Stay on this page, and no refresh.");
        ...
        var s = landing.query || {};
        s.action = landing.action || this.action;
...
console.log("start ajax..." + role + ", " + comp);
    var url = this.siteUrl+"/"+role+"/"+this.json+"/"+comp;
    if (query===undefined || query===null) query = {};
    if (query.action===undefined) query.action = this.action;
...
console.log("RAW: " + this.responseText);
          ...
          if (!data.success) {
            if (data.error_string=="challenge" || data.error_string=="failed") {
console.log("Login please.");
              that.names = data;
              that.updateParameters(role, that.logins);
            } else {
console.log("Website error: "+data.error_string);
